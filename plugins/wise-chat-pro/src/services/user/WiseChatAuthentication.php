...
class WiseChatAuthentication {
    ...
    public function getAnonymousActionLoginURL() {
        $parameters = array(
            'wcLoginAction' => 'an',
    ...
    public function handleAuthentication() {
        ...
        try {
            ...
            if ($method === 'an' && !$this->isAuthenticated() && $this->options->isOptionEnabled('anonymous_login_enabled', true)) {
                $user = $this->authenticateAnonymously();
            } else if ($method === 'un' && !$this->isAuthenticated() && $this->options->isOptionEnabled('force_user_name_selection', false)) {
    ...
    public function isAuthenticatedExternally() {
        ...
        return $user !== null && strlen($user->getExternalType()) > 0;
    }
